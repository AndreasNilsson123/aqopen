...
import { store, setCourses } from './store.js';
...
export function syncedNote() {
  if (!store.offline) return 'Sparat · alla som öppnar sidan ser samma resultat';
...
async function flush() {
  ...
  try {
    ...
    if (!res.conflict) { store.rev = res.doc.rev; store.base = clone(store.S); }
    store.offline = false;
